...
class Welcome extends MY_Controller
{
    function index()
    {
        ...
        $data_siswa = $this->siswa_model->retrieve_kelas_siswa($siswa_id);
        $data_kelas = $this->kelas_model->retrieve($data_siswa['kelas_id']);
        ...
        if (is_siswa()) {
            ...
            $data['materi_terbaru'] = $retrieve_all_materi['results'];
             ...
             # ambil materi kelas
            $retrieve_all_materi_kelas = $this->materi_model->retrieve_all(
                10,
                1,
                null,
                null,
                ...
                null,
                1,
                $data_kelas['parent_id']
            );
            $data['materi_kelas_terbaru'] = $retrieve_all_materi_kelas['results'];
            ...
            # ambil tugas kelas
            $retrieve_all_tugas_kelas = $this->tugas_model->retrieve_all(
                10,
                1,
                null,
                null,
            ...
            $data['tugas_kelas_terbaru'] = $retrieve_all_tugas_kelas['results'];
        ...
        if (is_admin()) {
            ...
            $semua_pengajar = $this->pengajar_model->retrieve_all_filter(
                '', '', array(), '', '', '', '', '', array(1, 2), '', '', 1, false
            );

            $rekap_guru = array();
            foreach ($semua_pengajar as $pg) {
                // jumlah tugas
                $this->db->where('pengajar_id', $pg['id']);
                $jml_tugas = $this->db->count_all_results('tugas');

                // jumlah tugas aktif
                $this->db->where('pengajar_id', $pg['id']);
                $this->db->where('aktif', 1);
                $jml_tugas_aktif = $this->db->count_all_results('tugas');

                // jumlah materi
                $this->db->where('pengajar_id', $pg['id']);
                $jml_materi = $this->db->count_all_results('materi');

                // tugas terbaru
                $this->db->where('pengajar_id', $pg['id']);
                $this->db->order_by('tgl_buat', 'DESC');
                $tugas_terbaru = $this->db->get('tugas', 1)->row_array();

                $rekap_guru[] = array(
                    'id'             => $pg['id'],
                    'nama'           => $pg['nama'],
                    'foto'           => $pg['foto'],
                    'jenis_kelamin'  => $pg['jenis_kelamin'],
                    'jml_tugas'      => $jml_tugas,
            ...
            // urutkan berdasarkan jumlah tugas terbanyak
            usort($rekap_guru, function($a, $b) {
                return $b['jml_tugas'] - $a['jml_tugas'];
            });

            $data['rekap_guru'] = $rekap_guru;
        }
    ...
    function siswa_online()
    {
        must_login();

        if (!is_admin()) redirect('welcome');

        // Ambil semua login_log yang last_activity dalam 15 menit terakhir
        $batas = time() - (15 * 60);
        $this->db->select('login_log.id, login_log.login_id, login_log.lasttime, login_log.last_activity, login_log.agent');
        $this->db->from('login_log');
        $this->db->join('login', 'login.id = login_log.login_id', 'inner');
        $this->db->where('login_log.last_activity >=', $batas);
        $this->db->where('login.siswa_id !=', 0);
        $this->db->order_by('login_log.last_activity', 'DESC');
        $logs = $this->db->get()->result_array();

        $siswa_online = array();
        foreach ($logs as $log) {
            $login = $this->db->get_where('login', array('id' => $log['login_id']))->row_array();
            if (empty($login['siswa_id'])) continue;

            $siswa = $this->siswa_model->retrieve($login['siswa_id']);
            if (empty($siswa)) continue;

            $kelas_siswa = $this->kelas_model->retrieve_siswa(null, array('siswa_id' => $siswa['id'], 'aktif' => 1));
            $kelas = !empty($kelas_siswa) ? $this->kelas_model->retrieve($kelas_siswa['kelas_id']) : null;

            $agent = json_decode($log['agent'], true);
            $siswa_online[] = array(
                'nama'          => $siswa['nama'],
                'nis'           => $siswa['nis'],
                'foto'          => $siswa['foto'],
        ...
        $data['siswa_online'] = $siswa_online;
        $data['jumlah']       = count($siswa_online);
    ...
    function dashboard_guru()
    {
        ...
        $this->twig->display('dashboard-guru.html', $data);
    }
