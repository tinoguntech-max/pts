...
class Tugas extends MY_Controller
{
    ...
    private function formatData($val)
    {
        ...
        # jumlah soal
        $val['jumlah_soal'] = $this->tugas_model->count_pertanyaan($val['id']);

        # persentase mengerjakan
        $total_siswa  = 0;
        $sudah_siswa  = count($this->tugas_model->retrieve_all_nilai($val['id']));
        $tugas_kelas  = $this->tugas_model->retrieve_all_kelas($val['id']);
        foreach ($tugas_kelas as $tk) {
            $siswa_kelas = $this->siswa_model->retrieve_siswa_kelas($tk['kelas_id']);
            $total_siswa += count($siswa_kelas);
        }
        $val['total_siswa'] = $total_siswa;
        $val['sudah_siswa'] = $sudah_siswa;
        $val['persen_mengerjakan'] = ($total_siswa > 0) ? round(($sudah_siswa / $total_siswa) * 100) : 0;
    ...
    function index($segment_3 = '')
    {
        ...
        $data['all_tugas']  = $this->tugas_model->getall();
    ...
    function leger($tugas_id = '')
    {
        
        $arytugas = $_POST['mdb-select'];
         ...
         foreach ($siswa as $key => $value) {
             ...
             $ary[$i]['id'] = $value['id'];
             $ary[$i]['nis'] = $value['nis'];
             $ary[$i]['nama'] = $value['nama'];
             $kelas = $this->kelas_model->getkelas($value['id']);
             $ary[$i]['kelas'] = $kelas[0]['nama'];
             $ii = 0;
             $arynilai =array();
             foreach ($arytugas as $key => $value_tugas) {
                $nilai = $this->tugas_model->getbytugas($value['id'], $value_tugas);
                if($nilai != 0){
                    $nilai = round( $nilai[0]['nilai'], 2);
                }
                else{
                	$check = $this->tugas_model->retrieve_all_kelas_bayu($value_tugas, $kelas[0]['id']); 
                	//if ($value['nis'] == '34018124') 
                //	var_dump($check);exit;
                	if($check != 0){
                    	$nilai = '-';
                	}
                 ...
                 $arynilai[$ii]['tugas_id'] = $value_tugas;
                 $arynilai[$ii]['nilai'] = $nilai;
                 $ii++;

             }
             $ary[$i]['nilai']= $arynilai;   
             $i++;
         }
         
        $i = 0;
        foreach ($arytugas as $key => $value) {
                $mapel = $this->tugas_model->retrieve($value);
                $kelas1 = $this->tugas_model->retrieve_all_kelas_bayu($value);
                $namakelas = '(';
                if($kelas1 != 0){
                	 foreach ($kelas1 as $key => $value1) {
                	 	$res = $this->kelas_model->retrieve($value1['kelas_id']);
                	 	$namakelas .= $res['nama'].',';
                	 }
                }
                $mapel = $this->mapel_model->retrieve($mapel['mapel_id']);
                $arytugas[$i] =  $value.'.'.$mapel['nama'].' '.$namakelas.')';
                $i++;            
        }   

        $data['all']  = $ary;
        $data['arytugas']  = $arytugas;
        header("Content-type: application/vnd-ms-excel");
        header("Content-Disposition: attachment; filename=nilai-all.xls");
        $this->twig->display('export-excel-nilai-all.html', $data);
    }
    ...
    function nilai($tugas_id = '', $mode = '')
    {
        ...
        $i = 0;
        $siswa = array();
        foreach ($data['tugas']['tugas_kelas'] as $tugas_kelas) {
          $kelasaktif = $this->siswa_model->retrieve_siswa_kelas($tugas_kelas['id']);
          foreach ($kelasaktif as $key => $value) {
            $siswa[$i]['id'] = $value['id'];
            $siswa[$i]['nis'] = $value['nis'];
            $siswa[$i]['nama'] = $value['nama'];
            $siswa[$i]['status_id'] = $value['status_id'];
            $siswa[$i]['kelas'] = $tugas_kelas['id'];
            $i++;
          }
        }
        ...
        if (is_pengajar() OR is_admin()) {
            ...
            $no1 = 1;
            $datafilter = array();
            foreach ($siswa as $key => $filter) {
                $key = array_search($filter['id'], array_column($retrieve_nilai, 'siswa_id'));
                
                
                if (!is_int($key)){
                     if (!empty($_POST['kelas_id'])) {
                        if ($_POST['kelas_id'] ==  $filter['kelas']) {                            
                            if($filter['status_id'] == 1){
                                if($no != 0){   
                                    $datafilter[$no]['no'] = $no1;
                                    $datafilter[$no]['id'] = $filter['id'];
                                    $datafilter[$no]['nis'] = $filter['nis'];
                                    $datafilter[$no]['nama'] = $filter['nama'];
                                    $kelas = $this->kelas_model->retrieve($filter['kelas']);
                                    $datafilter[$no]['kelas'] =  $kelas; 
                                    $datafilter[$no]['status'] =  "Belum";   
                       				$no1++;
                                }       
                        ...
                        if ($_POST['kelas_id'] ==  $filter['kelas']) {                            
                            if($filter['status_id'] == 1){
                                if($no != 0){   
                                    ...
                                    $kelas = $this->kelas_model->retrieve($filter['kelas']);
                                    $datafilter[$no]['kelas'] =  $kelas; 
                                    $datafilter[$no]['status'] =  "Mengerjakan"; 
                       				 $no1++;
                                }       
            ...
            $data['data_nilai_filter']  = $datafilter;

            if ($mode == 'print') {
                $this->twig->display('print-nilai.html', $data);
            }elseif($mode == 'print_bayu'){
                $data['comp_js'] = load_comp_js(array(
                    base_url('assets/comp/datatables/jquery.dataTables.js'),
                    base_url('assets/comp/datatables/datatable-bootstrap2.js'),
                    base_url('assets/comp/colorbox/jquery.colorbox-min.js'),
                ...
                $this->twig->display('list-mengerjakan.html', $data);
            } elseif ($mode == 'export_excel_bayu') {
                header("Content-type: application/vnd-ms-excel");
                header("Content-Disposition: attachment; filename=nilai-" . url_title($data['tugas']['judul'], '-', true)  . " mengerjakan.xls");
                $this->twig->display('export-excel-nilai-bayu.html', $data);
    ...
    function koreksi($tugas_id = '', $mode = '')
    {
        ...
        foreach ($retrieve_all_history as $history) {
                ...
                $retrieve_all_history[$i]["siswa_id"] = $siswa_id;
                $i++;
        }
        ...
        foreach ($siswa as $key => $filter) {
            ...
            $key = array_search($filter['id'], array_column($retrieve_all_history, 'siswa_id'));
    ...
    function force_finish($tugas_id = '', $siswa_id = '')
    {
        if (!is_admin() AND !is_pengajar()) {
            show_error("Akses ditolak.");
        }
        ...
        $tugas = $this->tugas_model->retrieve($tugas_id);
        if (empty($tugas)) redirect('tugas');

        $field_id    = 'mengerjakan-' . $siswa_id . '-' . $tugas_id;
        $check_field = retrieve_field($field_id);

        if (empty($check_field)) {
            $this->session->set_flashdata('tugas', get_alert('warning', 'Data pengerjaan siswa tidak ditemukan.'));
            redirect('tugas/pantau/' . $tugas_id);
        }

        $check_field_value = json_decode($check_field['value'], 1);

        # hitung nilai jika pilihan ganda
        if ($tugas['type_id'] == 3) {
            $jml_soal  = count($check_field_value['pertanyaan_id']);
            $jml_benar = 0;
            $jml_salah = 0;
            ...
            $this->tugas_model->create_nilai($nilai, $tugas_id, $siswa_id);
            $check_field_value['nilai']     = $nilai;
            $check_field_value['jml_benar'] = $jml_benar;
            $check_field_value['jml_salah'] = $jml_salah;
        } else {
            # essay: simpan history tanpa nilai dulu
            $check_field_value['nilai'] = 0;
        }
        ...
        $check_field_value['tgl_submit']   = $sekarang;
        $check_field_value['total_waktu']  = lama_pengerjaan($check_field_value['mulai'], $sekarang);
        $check_field_value['force_finish'] = true;

        $new_field_id = 'history-mengerjakan-' . $siswa_id . '-' . $tugas_id;
        create_field($new_field_id, 'History pengerjaan tugas', json_encode($check_field_value));

        $this->session->set_flashdata('tugas', get_alert('success', 'Nilai siswa berhasil diproses.'));
        redirect('tugas/pantau/' . $tugas_id);
    }

    function belum_mengerjakan($tugas_id = '')
    {
        if (!is_admin() AND !is_pengajar()) {
            show_error("Akses ditolak.");
        }
        ...
        $data['tugas']  = $this->formatData($tugas);
        $data['siswa']  = $this->tugas_model->retrieve_belum_mengerjakan($tugas_id);
        $data['jumlah'] = count($data['siswa']);

        $this->twig->display('belum-mengerjakan.html', $data);
    }
}
