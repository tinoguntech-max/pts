    ...
    function index($segment_3 = '')
    {
        ...
        if ($this->form_validation->run('materi/filter') == true) {
            ...
            $filter = array(
                ...
                'kelas_id_detil'    => $this->input->post('kelas_id_detil', true),
            );
...
//var_dump($retrieve_kelas);exit;
        ...
        $data['kelas_detil']      = $this->kelas_model->retrieve_all_child();
    ...
    function add($segment_3 = '')
    {
        ...
        if ($success) {
            ...
            $kelas_id = $this->input->post('kelas_id', TRUE);
            $kelas_id_detil = $this->input->post('kelas_id_detil', TRUE);
            foreach ($kelas_id_detil as $materi_kelas_id) {
                # cek sudah ada belum
	...
	function detail_view($segment_3 = '', $segment_4 = '')
    {
        ...
        # cari siswa membaca
        $siswa = $this->materi_model->retrieve_view($materi['id']);
        $siswa_all = $this->siswa_model->getall();
        ...
        foreach ($siswa as $row) {
            $data1 = $this->siswa_model->retrieve($row['user_id']);
            $kelas1data = $this->siswa_model->retrieve_kelas_siswa($row['user_id']);
            $kelas1 = $this->kelas_model->retrieve($kelas1data['kelas_id']);
          	if (!isset($kelas_view[$kelas1data['kelas_id']])) {
                $kelas_view[$kelas1data['kelas_id']] = $kelas1;
            }
            if (!empty($_POST['kelas_id'])) {
                if($_POST['kelas_id'] == $kelas1data['kelas_id']){
		           	$gak_eroh[$i]['no'] =  $i+1;
                    $gak_eroh[$i]['nis'] =  $data1['nis'];
		            $gak_eroh[$i]['id'] =  $row['user_id'];
		            $gak_eroh[$i]['nama'] =  $data1['nama'];
		            $gak_eroh[$i]['kelas'] =  $kelas1['nama'];
		            $i++;                	
                }
            }
            else{
        ...
        $result = array();
        $i = 0;
        foreach ($siswa_all as $row) {
            $data1 = $this->siswa_model->retrieve($row['id']);
            $kelas1data = $this->siswa_model->retrieve_kelas_siswa($row['id']);
            $kelas1 = $this->kelas_model->retrieve($kelas1data['kelas_id']);
            if (!isset($kelas_view[$kelas1data['kelas_id']])) {
                $kelas_view[$kelas1data['kelas_id']] = $kelas1;
            }
            if (!empty($_POST['kelas_id'])) {
                ...
                foreach ($gak_eroh as $value) {
                    if($row['id'] == $value['id']){
                        $status = "Sudah Membaca";    
                    }
                } 
                $result[$i]['no'] =  $i+1;
                $result[$i]['nis'] =  $data1['nis'];
                $result[$i]['id'] =  $row['id'];
                $result[$i]['nama'] =  $data1['nama'];
                $result[$i]['kelas'] =  $kelas1['nama'];
        ...
        $data['data_siswa'] = $result;
    	$data['kelas_view'] = $kelas_view;  
    	$data['materi'] = $materi;  

    	 if ($segment_4 == 'print_bayu') {
                ...
                $this->twig->display('list-view.html', $data);
         } elseif($segment_4 == 'excel_bayu'){
             header("Content-type: application/vnd-ms-excel");
             header("Content-Disposition: attachment; filename=data-siswa-" . url_title($data['materi']['judul'], '-', true)  . ".xls");
             $this->twig->display('export-excel-portable.html', $data);
         }
         else {
            # panggil datatables dan combobox
            $data['comp_js'] = load_comp_js(array(
    ...
    function detail($segment_3 = '', $segment_4 = '', $segment_5 = '')
    {
        ...
        if (is_pengajar() && $materi['publish'] == '0' && $materi['pengajar_id'] != get_sess_data('user', 'id')) {
         ...
         # tambah views manual
        $id_user = get_sess_data('login', 'siswa_id');
        if(!is_null($id_user)){
	        $check = $this->materi_model->retrieve_view($materi['id'], $id_user);
	        if(empty($check)) {
				$this->materi_model->create_view($materi['id'], $id_user);
			}
		}
