...
class Materi_model extends CI_Model
{
    ...
    public function create_view(
        ...
        $this->db->insert('materi_view', array(
            'materi_id' => $materi_id,
            'user_id'  => $user_id
        ));
        return $this->db->insert_id();
    ...
    public function retrieve_all(
        ...
        if (!empty($kelas_id) || !empty($kelas_id_detil)) {
            $where['materi_kelas']          = array('materi.id = materi_kelas.materi_id', 'join', 'inner');
            if (!empty($kelas_id)) {
             $where['materi_kelas.kelas_id'] = array($kelas_id, 'where_in');
            }
            if (!empty($kelas_id_detil)) {
             $where['materi_kelas.kelas_id_detil'] = array($kelas_id_detil, 'where_in');
            }
            $group_by[] = 'materi.id';
        }
        if (!empty($type)) {
    ...
    public function retrieve_view($id, $user = 0)
    {
        $id = (int)$id;
        ...
        $result = $this->db->get('materi_view');
        return $result->result_array();
    }
