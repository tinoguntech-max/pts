...
class Tugas_model extends CI_Model
{
   ...
   public function getall() {
    ...
    /**
     * Method untuk mendapatkan siswa yang belum mengerjakan tugas
     *
     * @param  integer $tugas_id
     * @return array
     */
    public function retrieve_belum_mengerjakan($tugas_id)
    {
        $tugas_id = (int)$tugas_id;

        // Ambil semua kelas yang terkait tugas ini
        $this->db->select('kelas_id');
        $this->db->where('tugas_id', $tugas_id);
        $tugas_kelas = $this->db->get('tugas_kelas')->result_array();

        if (empty($tugas_kelas)) return array();

        // Ambil semua nilai yang sudah ada
        $this->db->select('siswa_id');
        $this->db->where('tugas_id', $tugas_id);
        $sudah = $this->db->get('nilai_tugas')->result_array();
        $sudah_ids = array_column($sudah, 'siswa_id');

        // Ambil semua siswa dari kelas terkait pakai kelas_siswa
        $belum = array();
        foreach ($tugas_kelas as $tk) {
            $kelas_id = $tk['kelas_id'];

            $this->db->select('siswa.id, siswa.nama, siswa.nis, kelas.nama AS nama_kelas');
            $this->db->from('siswa');
            $this->db->join('kelas_siswa', 'kelas_siswa.siswa_id = siswa.id', 'inner');
            ...
            $belum = array_merge($belum, $result);
        }

        return $belum;
    }

   public function getbytugas($siswa, $tugas_id = 0) {
        $this->db->select('*');
        $this->db->from('nilai_tugas');
        $this->db->where('siswa_id', $siswa);
        if($tugas_id != 0)
            $this->db->where_in('tugas_id', $tugas_id);
        $query = $this->db->get();
     ...
     * @author Almazari <almazary@gmail.com>
     */
    public function retrieve_all_kelas_bayu($tugas_id = null, $kelas_id = null)
    {
        if (!empty($tugas_id)) {
            $this->db->where('tugas_id', $tugas_id);
        }
