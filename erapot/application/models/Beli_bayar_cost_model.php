<?php
class Beli_bayar_cost_model extends CI_Model {
	
	public function add($data) {
        $this->db->insert('beli_bayar_cost',$data);
    }
	
	/*public function update($data) {
		$array = array('id'=>$data['id']);
		$this->db->where($array)->update('hotline_cost', $data);
	}*/
	
	public function updatesisa($data) {
		$array = array('id_beli_bayar'=>$data['id_beli_bayar'], 'id_beli_terima'=>$data['id_beli_terima']);
		$this->db->where($array)->update('beli_bayar_cost', $data);
	}
	
	public function delete($id) {
		$this->db->delete('beli_bayar_cost', array('id_beli_bayar' => $id));
	}
    
	public function record_count() {
        return $this->db->count_all('beli_bayar_cost');
    }

    public function get($id) {
		$query = $this->db->where('id_beli_bayar', $id)->get('beli_bayar_cost');
	...
	/*public function getall() {
		$query = $this->db->order_by('tanggal', 'desc')->get('hotline_cost');
	...
	public function get($id) {
		$query = $this->db->where('id', $id)->get('hotline_cost', 1, 0);
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
			return 0;
	...
	/*public function getbybeliterima($id1, $id2) {
		$query = $this->db->where(array('id_beli_bayar'=>$id1, 'id_beli_terima'=>$id2))->get('beli_bayar_cost', 1, 0);
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
			return 0;
	...
	public function getbybeliterima($id) {
		$q = "SELECT * FROM beli_bayar_cost WHERE id_beli_terima='{$id}'";
	...
	public function getsumsisa($id) {
		$query = $this->db->query("select sum(jumlah_sisa) as toro from beli_order_cost where id_beli_order=".$id);
	...
	// public function laporan($tgl1 = null, $tgl2 = null, $tgl3 = null, $tgl4 = null, $cabang='0', $inv_no='0', $po_no='0', $bpb_no='0') {
		// $q = "SELECT * from beli_bayar_cost a, beli_bayar b, beli_terima c, beli_order d
	...
	public function getbybeliordersupplierforbayar($supplier, $cabang = null) {		
		...
		$this->db->join('beli_bayar', 'beli_bayar.id = beli_bayar_cost.id_beli_bayar');
		$where = array('beli_bayar.id_supplier'=>$supplier, 'beli_bayar_cost.status'=>0);
		$query = $this->db
			->where($where)
			->get('beli_bayar_cost');
		if ($query->num_rows() > 0)
	...
	public function laporan($tgl1 = null, $tgl2 = null, $tgl3 = null, $tgl4 = null, $cabang='0', $inv_no='0', $po_no='0', $bpb_no='0') {
		...
		$this->db->select('*');
		$this->db->from('beli_bayar_cost');
		$this->db->join('beli_bayar', 'beli_bayar.id = beli_bayar_cost.id_beli_bayar', 'left outer');
		$this->db->join('beli_terima', 'beli_terima.id = beli_bayar_cost.id_beli_terima');
