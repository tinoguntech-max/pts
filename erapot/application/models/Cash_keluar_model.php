<?php
class Cash_keluar_model extends CI_Model {
	
	public function add($data) {
		$this->db->trans_start();
        $this->db->insert('cash_keluar',$data);
	...
	public function update($data) {
		...
		$this->db->where($array)->update('cash_keluar', $data);
	}
    
	public function record_count() {
        return $this->db->count_all('cash_keluar');
    }

	public function getall() {
		$query = $this->db->order_by('tanggal', 'desc')->get('cash_keluar');
	...
	public function get($id) {
		$query = $this->db->where('id', $id)->get('cash_keluar', 1, 0);
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
			return 0;
