...
class Stock_model extends CI_Model {
	...
	public function updatebybarang($data) {
		$array = array('id_barang'=>$data['id_barang'], 'id_m_cabang'=>$data['id_m_cabang']);
		$this->db->where($array)->update('stock', $data);
	}
	
	public function deletebytransaction($type, $id) {
		$this->db->delete('stock', array('keterangan' => $type, 'id_trans' => $id));
	}
	...
	public function getlastpenerimaanbybarang($id) {
		$query = $this->db->where('keterangan','PENERIMAAN')->where('id_barang', $id)->order_by('tanggal', 'desc')->get('stock', 1, 0);
	...
	public function getfirststockawalbybarang($id) {
		$query = $this->db->where('keterangan','STOCK_AWAL')->where('id_barang', $id)->order_by('tanggal', 'asc')->get('stock', 1, 0);
	...
	// mendapatkan semua barang, primarily digunakan stock updater in-out
	public function getallbybarang($id_barang, $cabang) {
		$query = $this->db->where(array('id_barang'=>$id_barang, 'id_m_cabang'=>$cabang))->order_by('tanggal asc, id asc')->get('stock');
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
			return 0;
	...
	public function getstockawal($id, $tanggal, $cabang='0') {
		$q = "SELECT max(id) as toro FROM stock WHERE id_barang = '".addslashes($id)."' AND date(tanggal) < '{$tanggal}'";
	...
	public function getbyketeranganandtrans($id, $id1) {
		$query = $this->db->where(array('id_trans'=>$id, 'keterangan'=>$id1))->get('stock');
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
			return 0;
	...
	public function getrowsaffected($id_row, $id_barang, $tanggal, $cabang = 0) {
		$where = "(date(tanggal) > '{$tanggal}' OR (date(tanggal) = '{$tanggal}' AND id >= {$id_row})) AND id_barang = '".addslashes($id_barang)."' 
	...
	public function getrowsaffectedpenjualan($id_row, $id_barang, $tanggal, $cabang = 0) {
		$where = "(date(tanggal) > '{$tanggal}' OR (date(tanggal) = '{$tanggal}' AND id > {$id_row})) AND id_barang = '".addslashes($id_barang)."' AND keterangan = 'PENJUALAN' 
			AND id_m_cabang = {$cabang}";
		$query = $this->db->where($where)->order_by('tanggal asc, id asc')->get('stock');
		if ($query->num_rows() > 0)
			return $query->result_array();
	...
	public function getnotsellingyet($id_barang, $cabang=0) {
		$where = "id_barang = '".addslashes($id_barang)."' AND id_m_cabang={$cabang} AND sell_left > 0";
	...
	public function sumnotsellingyet($id_barang) {
		$query = $this->db->query("select sum(sell_left) as toro from stock where id_barang='".addslashes($id_barang)."'");
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
			return 0;
	...
	public function sumnotsellingyetbranch($id_barang, $branch) {
		$query = $this->db->query("select sum(sell_left) as toro from stock where id_barang='".addslashes($id_barang)."' and id_m_cabang='".$branch."'");
		if ($query->num_rows() > 0)
			return $query->result_array();
		else
			return 0;
	...
	public function laporan($tgl1 = null, $tgl2 = null, $barang='0', $cabang='0') {
		...
		if ($barang != '0')
			$q .= " AND id_barang = '".addslashes($barang)."'";
		$q .= " AND id_m_cabang = {$cabang}";
