...
<div class="modal fade" id="toroModal1" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	...
	<script>
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_beli/laporan_beli_minta_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected), no_faktur: encodeURIComponent(no_faktur), tgl3: a, tgl4: b, 
						status: encodeURIComponent(selected1) })
				.done(function( data ) {
