...
<script>
		...
		function generate_laporan() {
				...
				$.get("<?php echo base_url("laporan_pos"); ?>/laporan_pos_jual_delivery_detil_ajax/"+moment(startDate).format('YYYY-MM-DD')+
					"/"+moment(endDate).format('YYYY-MM-DD')+"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(inv_no)+"/"+encodeURIComponent(selected1), function(data) {
					$("#toro-area").html(data);
