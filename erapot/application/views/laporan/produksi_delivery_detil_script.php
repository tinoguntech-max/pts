...
<script>
		...
		function generate_laporan() {
				...
				$.get("<?php echo base_url("laporan_produksi"); ?>/laporan_produksi_delivery_detil_ajax/"+moment(startDate).format('YYYY-MM-DD')+
					"/"+moment(endDate).format('YYYY-MM-DD')+"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(selected1)+"/"+encodeURIComponent(rp_no)+"/"+encodeURIComponent(kendaraan_nama)+"/"+encodeURIComponent(kendaraan_nopol)+"/"+encodeURIComponent(kendaraan_sopir), function(data) {
					$("#toro-area").html(data);
