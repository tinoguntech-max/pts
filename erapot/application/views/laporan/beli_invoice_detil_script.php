	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h3 class="modal-title" id="myModalLabel">Surat Invoice Barang</h3>
				<?= CLIENT_NAME ?><br/>
				<?= CLIENT_ADDRESS ?>
			</div>
...
<div class="modal fade" id="toroModal1" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h3 class="modal-title" id="myModalLabel">Surat Penerimaan Pembelian</h3>
				<?= CLIENT_NAME ?><br/>
				<?= CLIENT_ADDRESS ?>
			</div>
			<div class="modal-body" id="toroModalBody">
...
<div class="modal fade" id="toroModal2" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h3 class="modal-title" id="myModalLabel">Surat Pembayaran Utang</h3>
				<?= CLIENT_NAME ?><br/>
				<?= CLIENT_ADDRESS ?>
			</div>
			<div class="modal-body" id="toroModalBody">
...
<script>
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_beli/laporan_beli_invoice_detil_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected), inv_no: encodeURIComponent(inv_no), po_no: encodeURIComponent(po_no), bpb_no: encodeURIComponent(bpb_no), 
						tgl3: a, tgl4: b })
				.done(function( data ) {
		...
		$(document).ready(function() {
			...
			$("#toroModal2").on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				$.ajax({
					url: "<?php echo base_url("beli_bayar"); ?>/ajax_detil",
					data: {
						q: recipient
					},
					success: function( data ) {
