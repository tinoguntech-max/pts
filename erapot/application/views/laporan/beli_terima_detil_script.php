...
<script>
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_beli/laporan_beli_terima_detil_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						barang: encodeURIComponent(selected), cabang: encodeURIComponent(selected1), bpb_no: encodeURIComponent(bpb_no) })
				.done(function( data ) {
