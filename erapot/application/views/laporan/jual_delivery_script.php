	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h3 class="modal-title" id="myModalLabel">Surat Pengiriman</h3>
				<?= CLIENT_NAME ?><br/>
				<?= CLIENT_ADDRESS ?>
			</div>
...
<script>
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_jual/laporan_jual_delivery_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected), do_no: encodeURIComponent(do_no), 
						kendaraan_nama: encodeURIComponent(kendaraan_nama), kendaraan_nopol: encodeURIComponent(kendaraan_nopol), kendaraan_sopir: encodeURIComponent(kendaraan_sopir) })
				.done(function( data ) {
		...
		$(document).ready(function() {
			...
			$("#toroModal").on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				$.ajax({
					url: "<?php echo base_url("jual_delivery"); ?>/ajax_detil",
					data: {
						q: recipient
					},
			...
			$("#toroModal2").on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				$.ajax({
					url: "<?php echo base_url("jual_retur"); ?>/ajax_detil",
					data: {
						q: recipient
					},
					success: function( data ) {
