...
<script>
		...
		function generate_laporan() {
				...
				var brands3 = $('#murid option:selected');
				var selected3 = [];
				...
				$.get("<?php echo base_url("laporan_pos"); ?>/laporan_pos_beli_terima_detil_ajax/"+moment(startDate).format('YYYY-MM-DD')+
					"/"+moment(endDate).format('YYYY-MM-DD')+"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(inv_no)+"/"+encodeURIComponent(selected1)+"/"+encodeURIComponent(selected2)+"/"+encodeURIComponent(selected3), function(data) {
					$("#toro-area").html(data);
