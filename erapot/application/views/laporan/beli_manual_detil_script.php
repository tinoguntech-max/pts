...
<script>
		...
		function generate_laporan() {
				var brands1 = $('#m_cabang option:selected');
				var selected1 = [];
				...
				supplier = supplier.replace(/\//g,'-o-');
				
				var barang = $('#barang').val();
				if (barang == '') barang = '0';
				barang = barang.replace(/\//g,'-o-');

				$.get("<?php echo base_url("laporan_beli"); ?>/laporan_beli_manual_detil_ajax/"+moment(startDate).format('YYYY-MM-DD')+
					"/"+moment(endDate).format('YYYY-MM-DD')+"/"+encodeURIComponent(selected1)+"/"+encodeURIComponent(no_faktur)+"/"+encodeURIComponent(supplier)+"/"+encodeURIComponent(barang), function(data) {
					$("#toro-area").html(data);
		...
		$(document).ready(function() {
			...
			$("#toroModal").on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				$.ajax({
					url: "<?php echo base_url("beli_manual"); ?>/ajax_detil",
					data: {
						q: recipient
					},
