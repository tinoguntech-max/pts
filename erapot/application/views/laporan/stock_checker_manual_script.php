...
<script>
		...
		function updaterForm() {
			$( "#btn-submit-updater" ).prop( "disabled", true );
			$( "#btn-submit-updater" ).html('<i class="fa fa-spinner fa-pulse fa-fw"></i> Loading...');
			var check = true;
			...
			if (check == false) {
				...
				$( "#btn-submit-updater" ).html('Hitung Ulang');
				$( "#btn-submit-updater" ).prop( "disabled", false );
			}
			else regenerate_stock();
		}
		
		function regenerate_stock() {
				var brands = $('#m_cabang option:selected');
				var selected = [];
				...
				var brands2 = $('#barang option:selected');
				var selected2 = [];
				...
				$.post( "<?= base_url("stock_checker/regenerate_stock_ajax") ?>", 
					{ cabang:  selected, barang: selected2 })
				.done(function( data ) {
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("stock_checker/laporan_stock_checker_ajax") ?>", 
					{ cabang:  selected, barang: selected2 })
				.done(function( data ) {
					$("#toro-area").html(data.report);
					$("#n_record").html(data.n_record);
