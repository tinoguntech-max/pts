...
<script>
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_jual/laporan_jual_delivery_detil_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected), barang: encodeURIComponent(selected2), do_no: encodeURIComponent(do_no) })
				.done(function( data ) {
