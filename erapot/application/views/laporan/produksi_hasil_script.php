...
<div class="modal fade" id="toroModal1" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	...
	<script>
		...
		function generate_laporan() {
				...
				$.get("<?php echo base_url("laporan_produksi"); ?>/laporan_produksi_hasil_ajax/"+moment(startDate).format('YYYY-MM-DD')+"/"+moment(endDate).format('YYYY-MM-DD')+
				"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(no_faktur)+"/"+encodeURIComponent(selected1), function(data) {
					$("#toro-area").html(data);
