...
<script>
		...
		function generate_laporan() {
			...
			$.post( "<?= base_url("laporan_jual/laporan_jual_invoice_detil_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected), inv_no: encodeURIComponent(inv_no), so_no: encodeURIComponent(so_no), do_no: encodeURIComponent(do_no), 
						tgl3: a, tgl4: b })		
				.done(function( data ) {
