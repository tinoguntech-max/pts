...
<script>
		...
		function validateForm() {
			...
			// check selected
			var brands = $('#supplier option:selected');
			var selected = [];
			if (brands.length > 8) {
				check = false;
				error += 'Anda hanya bisa memilih maksimal 8 supplier<br/>';
			}
			// check selected
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_beli/laporan_beli_order_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						supplier: encodeURIComponent(selected), cabang: encodeURIComponent(selected1), po_no: encodeURIComponent(po_no), 
						tgl3: a, tgl4: b })
				.done(function( data ) {
					...
					$("#n_price").html(data.n_price);
					$("#n_delivery").html(data.n_delivery);
