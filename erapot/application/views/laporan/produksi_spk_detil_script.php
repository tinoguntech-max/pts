...
<script>
		...
		function validateForm() {
			...
			var brands3 = $('#barang option:selected');
			var selected3 = [];
			/*if (brands.length == 0) {
				check = false;
				error += 'Anda harus memilih minimal satu cabang<br/>';
		...
		function generate_laporan() {
				...
				$.get("<?php echo base_url("laporan_produksi"); ?>/laporan_produksi_spk_detil_ajax/"+moment(startDate).format('YYYY-MM-DD')+
					"/"+moment(endDate).format('YYYY-MM-DD')+"/"+moment(startDate1).format('YYYY-MM-DD')+"/"+moment(endDate1).format('YYYY-MM-DD')+
					"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(selected1)+"/"+encodeURIComponent(selected2)+"/"+encodeURIComponent(selected3)+"/"+encodeURIComponent(rp_no)+"/"+encodeURIComponent(n_r), function(data) {
					$("#toro-area").html(data);
