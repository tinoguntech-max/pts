<div class="modal fade" id="toroModal" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	...
	<script>
		...
		function validateForm() {
			...
			// check selected
			var brands = $('#siswa option:selected');
			var selected = [];
			if (brands.length == 0) {
				check = false;
				error += 'Anda harus memilih minimal satu Siswa<br/>';
			}
			
			var brands2 = $('#tugas option:selected');
			var selected2 = [];
			if (brands2.length == 0) {
				check = false;
				error += 'Anda harus memilih minimal satu Tugas<br/>';
			}
			error += '</div>';
			if (check == false) {
		...
		function generate_laporan() {
				...
				var brands1 = $('#mapel option:selected');
				var selected1 = [];
				...
				$.get("<?php echo base_url("laporan"); ?>/laporan_rapot_ajax/"+encodeURIComponent(selected)+"/"+encodeURIComponent(selected1)+"/"+encodeURIComponent(selected2), function(data) {
					$("#toro-area").html(data);
