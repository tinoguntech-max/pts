<div class="modal fade" id="toroModal" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	...
	<script>
		...
		function generate_laporan() {
				...
				$.get("<?php echo base_url("laporan_pos"); ?>/laporan_pos_beli_terima_ajax/"+moment(startDate).format('YYYY-MM-DD')+"/"+moment(endDate).format('YYYY-MM-DD')
				+"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(inv_no)+"/"+encodeURIComponent(selected1)+"/"+encodeURIComponent(selected2), function(data) {
					$("#toro-area").html(data);
