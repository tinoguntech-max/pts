...
<div class="modal fade" id="toroModal1" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h3 class="modal-title" id="myModalLabel">Surat Produksi Work Order</h3>
				<?= CLIENT_NAME ?><br/>
				<?= CLIENT_ADDRESS ?>
			</div>
			<div class="modal-body" id="toroModalBody">
...
<div class="modal fade" id="toroModal2" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
				<h3 class="modal-title" id="myModalLabel">Surat Produksi Retur</h3>
				<?= CLIENT_NAME ?><br/>
				<?= CLIENT_ADDRESS ?>
			</div>
			<div class="modal-body" id="toroModalBody">
	...
	<script>
		...
		function generate_laporan() {
				...
				if (no_faktur == '') no_faktur = '0';
				no_faktur = no_faktur.replace(/\//g,'-o-');
				
				var kendaraan_nama = $('#kendaraan_nama').val();
				if (kendaraan_nama == '') kendaraan_nama = '0';
				kendaraan_nama = kendaraan_nama.replace(/\//g,'-o-');
				
				var kendaraan_nopol = $('#kendaraan_nopol').val();
				if (kendaraan_nopol == '') kendaraan_nopol = '0';
				kendaraan_nopol = kendaraan_nopol.replace(/\//g,'-o-');
				
				var kendaraan_sopir = $('#kendaraan_sopir').val();
				if (kendaraan_sopir == '') kendaraan_sopir = '0';
				kendaraan_sopir = kendaraan_sopir.replace(/\//g,'-o-');
				...
				$.get("<?php echo base_url("laporan_produksi"); ?>/laporan_produksi_delivery_ajax/"+moment(startDate).format('YYYY-MM-DD')+"/"+moment(endDate).format('YYYY-MM-DD')
				+"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(no_faktur)+"/"+encodeURIComponent(kendaraan_nama)+"/"+encodeURIComponent(kendaraan_nopol)+"/"+encodeURIComponent(kendaraan_sopir)+"/"+encodeURIComponent(selected1), function(data) {
					$("#toro-area").html(data);
		...
		$(document).ready(function() {
			...
			$("#toroModal2").on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				$.ajax({
					url: "<?php echo base_url("produksi_retur"); ?>/ajax_detil",
					data: {
						q: recipient
					},
					success: function( data ) {
