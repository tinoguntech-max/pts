	...
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<div class="modal-header">
				...
				<h3 class="modal-title" id="myModalLabel">Surat Retur Pembelian</h3>
				<?= CLIENT_NAME ?><br/>
				<?= CLIENT_ADDRESS ?>
			</div>
...
<script>
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_beli/laporan_beli_retur_detil_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected), barang: encodeURIComponent(selected1), rb_no: encodeURIComponent(rb_no)})
				.done(function( data ) {
		...
		$(document).ready(function() {
			...
			$("#toroModal").on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				$.ajax({
					url: "<?php echo base_url("beli_retur_batch"); ?>/ajax_detil",
					data: {
						q: recipient
					},
					success: function( data ) {
