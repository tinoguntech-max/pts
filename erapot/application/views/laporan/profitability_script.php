...
<script>
		...
		function generate_laporan() {
				...
				$.post( "<?= base_url("laporan_jual/laporan_profitability_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected) })
				.done(function( data ) {
					$("#toro-area").html(data.report);
					$("#n_record").html(data.n_record);
					$("#t_quantity").html(data.t_quantity);
					$("#t_hpp").html(data.t_hpp);
					$("#t_profit").html(data.t_profit);
					$("#t_harga_jual").html(data.t_harga_jual);
