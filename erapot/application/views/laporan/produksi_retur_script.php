...
<script>
		...
		function generate_laporan() {
				...
				$.get("<?php echo base_url("laporan_produksi"); ?>/laporan_produksi_retur_ajax/"+moment(startDate).format('YYYY-MM-DD')+
					"/"+moment(endDate).format('YYYY-MM-DD')+"/"+encodeURIComponent(selected)+"/"+encodeURIComponent(rp_no), function(data) {
					$("#toro-area").html(data);
