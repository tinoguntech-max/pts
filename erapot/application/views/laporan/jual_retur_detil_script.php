...
<script>
		...
		function generate_laporan() {
				...
				var rj_no = $('#rj_no').val();
				if (rj_no == '') rj_no = '0';
				rj_no = rj_no.replace(/\//g,'-o-');
				
				$.post( "<?= base_url("laporan_jual/laporan_jual_retur_detil_ajax") ?>", 
					{ tgl1: moment(startDate).format('YYYY-MM-DD'), tgl2: moment(endDate).format('YYYY-MM-DD'),
						cabang: encodeURIComponent(selected), barang: encodeURIComponent(selected1), rj_no: encodeURIComponent(rj_no)})
				.done(function( data ) {
		...
		$(document).ready(function() {
			...
			$("#toroModal").on('show.bs.modal', function (event) {
				...
				var modal = $(this);
				$.ajax({
					url: "<?php echo base_url("jual_retur_batch"); ?>/ajax_detil",
					data: {
						q: recipient
					},
