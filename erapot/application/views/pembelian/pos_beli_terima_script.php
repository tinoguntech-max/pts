<script>
	...
	function OrganizerViewModel() {
		...
		self.totalSub = ko.computed(function() {
			...
			if(self.PPNstatus() == 1)
				total += total * (parseFloat(self.PPNvalue())/100 );
		...
		$("#city").autocomplete({
			...
			select: function( event, ui ) {
				...
				if (rows == true)
					self.organizers.push(new AddOrganizers(ui.item.label, ui.item.value, ui.item.satuan, 1, ui.item.harga, <?= get_data('general','id',14,'nilai') ?>, 0 ));
				$("#city").val( '' );
				return false;
			},
					...
					$.ajax({
						...
						success: function( data ) {
							if (data != '' && data != null) {
								...
								if (rows == true)
									self.organizers.push(new AddOrganizers(data.label, data.value, data.satuan, 1, data.harga, <?= get_data('general','id',14,'nilai') ?>, 0));
								$("#city").val( '' );
							}
						}
					});
		...
		$("#supplier_name").autocomplete({
			source: function( request, response ) {
				$.ajax({
					url: "<?php echo base_url("pos_beli_terima"); ?>/auto_supplier",
					dataType: "json",
					data: {
						q: request.term
					},
