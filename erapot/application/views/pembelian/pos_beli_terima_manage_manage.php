...
<div class="modal fade" id="toroModalCanceled" tabindex="-1" role="dialog" aria-labelledby="toroModalLabel">
	<div class="modal-dialog modal-lg" role="document">
		<div class="modal-content">
			<form id="toroForm" method="post" action="<?= base_url('beli_order/canceled') ?>">
			<div class="modal-header">
				<button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
...
$(document).ready(function(){
	...
	$("#toroModalCanceled").on('show.bs.modal', function (event) {
		var button = $(event.relatedTarget);
		var recipient = button.data('pp');
		var modal = $(this);
		$("#toroForm").attr("action", "<?= base_url('pos_beli_terima_manage/canceled') ?>/" + recipient);
	});
});
</script>