...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function get_layout()
	{
		...
		if($this->echo_and_die === false)
		{
			...
			$js_vars =  array(
					'default_assets_path'		=> base_url().$this->default_assets_path,
					'default_root_assets'		=> base_url().'/assets',
	...
	protected function get_text_input($field_info,$value)
	{
		if($field_info->extras == 'text_editor')
		{
			...
			switch ($editor) {
				...
				case 'tinymce2':
					$this->set_js_lib($this->default_texteditor_path.'/tiny_mce2/jquery.tinymce.min.js');
					$this->set_js_config($this->default_javascript_path.'/jquery_plugins/config/jquery.tine_mce2.config.js');
				break;
...
class grocery_CRUD_States extends grocery_CRUD_Layout
{
                    ...
                    else
                    {
                        if (is_array($_POST['search_field'])) {
                            ...
                            foreach ($_POST['search_field'] as $search_key => $search_field_name) {
                                if (in_array($search_field_name, $this->toro_search_table)) {
									$toro_table = $this->basic_db_table.'.'.$search_field_name;
								}
								else
									$toro_table = $search_field_name;
								$search_array[$toro_table] = !empty($_POST['search_text'][$search_key]) ? $_POST['search_text'][$search_key] : '';
                            }
	...
	protected function getStatusUrl($primary_key = null)
	{
		if($primary_key === null)
			return $this->state_url('status');
		else
			return $this->state_url('status/'.$primary_key);
	}
}
...
class Grocery_CRUD extends grocery_CRUD_States
{
	...
	/* The unsetters */
	protected $toro_add				= null;
	protected $toro_edit			= null;
	protected $toro_view			= null;
	protected $toro_delete			= null;
	protected $toro_upload			= null;
	protected $toro_upload_link		= null;
	protected $toro_search_table	= array();
	protected $toro_tooltip			= array();
	...
	public function toro_add($link) {
		$this->toro_add = $link;
		return $this;
	}
	
	public function toro_edit($link) {
		$this->toro_edit = $link;
		return $this;
	}
	
	public function toro_view($link) {
		$this->toro_view = $link;
		return $this;
	}
	
	public function toro_delete($link) {
		$this->toro_delete = $link;
		return $this;
	}
	
	public function toro_upload($link) {
		$this->toro_upload = $link;
		return $this;
	}
	
	public function toro_upload_link($link) {
		$this->toro_upload_link = $link;
		return $this;
	}
	
	public function toro_search_table()
	{
		$args = func_get_args();
		...
		foreach($args as $arg)
		{
			$this->toro_search_table[] = $arg;
		}

		return $this;
	}
	
	public function toro_tooltip($field_name, $toro_tooltip = null)
	{
		$this->toro_tooltip[$field_name] = $toro_tooltip;
		return $this;
	}
