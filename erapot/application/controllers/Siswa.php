...
class Siswa extends CI_Controller {
	...
	public function index()
	{
		...
		$viewoptions['page'] = "siswa";
		$viewoptions['pageinfo'] = "List Siswa";
	...
	function toro_actions($value, $row) {
		...
		//$res = $this->access_group_detil_model->getbynama($this->session->userdata('user_level'),$cname.'_read');
		//if ($res != 0) {
			$print .= '<a class="btn btn-default btn-xs" href="'.site_url('siswa/detils').'/'.base64_encode($row->id).
	...
	public function detils($id = NULL){
		...
		foreach ($tugas_kelas as $thor => $value) {
			...
			$tugas_nilai = $this->nilai_tugas_model->getbytugas($id, $value['tugas_id']);
		...
		foreach ($mapel as $thor => $value_mapel) {
			$ary_value[$i]['id'] = $value_mapel['id'];
			$ary_value[$i]['nama'] = $value_mapel['nama'];
			$ary_value[$i]['detil'] = array();
			$ii = 0;
			foreach ($ary as $thor => $value_tugas) {
				if($value_tugas['mapel_id'] == $value_mapel['id']){
		...
		$data['viewoptions']['page'] = "siswa";
		$data['viewoptions']['pageinfo'] = "View Nilai";
		$data['viewoptions']['content'] = "new/nilai_print";
